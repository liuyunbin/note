...
public:
    Observer(const std::weak_ptr<ObserverPool>& s);
    ~Observer();
...
private:
    std::weak_ptr<ObserverPool> observer_pool_;
};
...
Observer::Observer(const std::weak_ptr<ObserverPool>& s) {
...
void Observer::add_observer_pool() {
    ...
    std::shared_ptr<ObserverPool> obj(observer_pool_.lock());
    if (obj)
        obj->add_observer(shared_from_this());
    else
        std::cout << "observer pool been delete" << std::endl;
}
...
auto observer_pool = std::make_shared<ObserverPool>();

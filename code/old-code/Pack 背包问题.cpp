...
public:
	...
	Pack(): money(0), money_first(0), money_second(0), count(0), expense(0), expense_first(0),
		expense_second(0), value(0), benefit(0), max_count(0), number(0){}
	...
	void multiple_pack(const int& expense, const int& value, int max_count)
	{
		...
		for (int j = 1; j < max_count + 1; max_count -= j, j *= 2)
		{
			for (int k = money; k != j * expense - 1; --k)
				benefit[k] = max (benefit[k], benefit[k - j * expense] + j * value);
		}
	}
		...
		for (int i = 0; i != count; ++i)
		{
			for (int j = expense[i]; j != money + 1; ++j)
			{
				if (benefit[j - expense[i]] != 0 || j - expense[i] == 0)
					benefit[j] = max (benefit[j], benefit[j - expense[i]] + value[i]);
			}
		}
		cout << "最高价值：" << benefit[money] << endl;
		delete[] benefit;
	...
	void P04()
	{
		...
		for (int i = 0; i != count; ++i)
		{
			cout << "请输入第" << i + 1 << "件物品的价格和价值和使用次数（-1 代表无数）： ";
		...
		for (int i = 0; i != count; ++i)
		{
			switch (max_count[i])
			{
			case 1: zero_one_pack(expense[i], value[i]); break;
			case -1: complete_pack(expense[i], value[i]); break;
			default: multiple_pack(expense[i], value[i], max_count[i]); break;
			}
		}
		cout << "最高价值： " << benefit[money] << endl;
		delete[] benefit;
	...
	void P05()
	{
		...
		for (int i = 0; i != count; ++i)
		{
			cout << "请输入第" << i + 1 << "件物品的第一种代价，第二种代价和价值： ";
		...
		for (int i = 0; i != count; ++i)
		{
			for (int j = money_first; j > expense_first[i] - 1; --j)
			{
				for (int k = money_second; k > expense_second[i] - 1; --k)
					benefit[j][k] = max (benefit[j][k], 
										benefit[j - expense_first[i]][k - expense_second[i]] + value[i]);
			}
		}
	...
	void P06()
	{
		...
		for (int i = 1; i != count + 1; ++i)
		{
			...
			for (int k = money; k != 0; --k)
			{
				...
				{
					if (k >= expense[j])
						benefit[k] = max (benefit[k], benefit[k - expense[j]] + value[j]);
				}
			}
			delete[] expense;
			delete[] value;
	...
	void P07()
	{
		...
		for (int i = 1; i != count + 1; ++i)
		{
			if (a[i][0] != 0)
			{
				...
				{
						benefit[j] = max (benefit[j], benefit[j - a[i][0]] + b[i][0]);
						if (j >= a[i][0] + a[i][1])
							benefit[j] = max (benefit[j], benefit[j - a[i][0] - a[i][1]] + b[i][0] + b[i][1]);
						if (j >= a[i][0] + a[i][2])
							benefit[i] = max (benefit[j], benefit[j - a[i][0] - a[i][2]] + b[i][0] + b[i][2]);
						if (j >= a[i][0] + a[i][1] + a[i][2])
							benefit[j] = max (benefit[j],
								benefit[j - a[i][0] - a[i][1] - a[i][2]] + b[i][0] + b[i][1] + b[i][2]);
				}
			}
		}
		cout << "最大价值： " << benefit[money] << endl;
...
int main()
{
	...
	while (true)
	{
		...
		switch (choose)
		{
		case 1: pack.P01(); break;
		case 2: pack.P02(); break;
		case 3: pack.P03(); break;
		case 4: pack.P04(); break;
		case 5: pack.P05(); break;
		case 6: pack.P06(); break;
		case 7: pack.P07(); break;
		case 8: pack.P022(); break;

...
int main(int argc, char* argv[]) {
    ...
    int client_socket = socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK,
                               IPPROTO_TCP);  // 非阻塞 socket
    ...
    std::cout << "try to connect " << server_ip << ":" << server_port
              << std::endl;
    int ret =
    ...
    if (ret == 1 && (event.events & EPOLLOUT)) {
        ...
        if (getsockopt(client_socket, SOL_SOCKET, SO_ERROR, &err, &err_len) <
                0 ||
            err != 0)
            std::cout << "connect error: " << std::endl;
        else

...
ctime            # 状态修改时间
                 ...
                 #   所有者, 所属组变化: chown
                 #   权限修改: chmod
stat -c %x 1.txt # 查看 atime
stat -c %y 1.txt # 查看 mtime
stat -c %z 1.txt # 查看 ctime
...
chmod g+s  filename        # 设置 SGID 权限
                           ...
                           #   有可执行权限时, 展示 s
chmod 1755 filename
chmod o+t  filename        # 设置 SBIT, 只有目录的所有者才能够删除目录中的文件
                           # 只针对目录生效
                           # 此目录下的文件只有自己可以删除
                           # 没有可执行权限时, 展示 T
                           #   有可执行权限时, 展示 t

chown lyb:lyb 1.txt        # 修改文件所属的组和用户
...
awk ..... lyb=$SHELL  # 将外部变量的值传给 awk, 在 BEGIN 块不起作用
...
killall -r vim     # 使用扩展的正则表达式, 包括 通配符?
...
stat       1.txt # 列出 birth atime mtime ctime
stat -L    1.txt # 符号链接所指向的文件, 而不是符号链接本身

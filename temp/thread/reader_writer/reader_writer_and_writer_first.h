#ifndef WRITER_WRITER_AND_WRITER_FIRST_H_
#define WRITER_WRITER_AND_WRITER_FIRST_H_
...
template <typename T>
class ReaderWriterAndWriterFirst {
 public:
  ...
  ReaderWriterAndWriterFirst() {
  }

  ReaderWriterAndWriterFirst(const ReaderWriterAndWriterFirst& rhs) {
    {
  ...
  void swap(ReaderWriterAndWriterFirst& rhs) {
    using std::swap;
    {
...
#else
      std::cerr << "ReaderWriterAndWriterFirst need c++11 or later" << std::endl;
  ...
  ReaderWriterAndWriterFirst& operator=(const ReaderWriterAndWriterFirst& rhs) {
    auto temp(rhs);
    swap(temp);
    return *this;
  }
  ...
  void Writer(const T& data) {
    {
        ...
        ++count_writer_wait_;
        wait_writer_.wait(lock, [this]{ return count_reader_run_ + count_writer_run_ == 0; });
        --count_writer_wait_;
        ++count_writer_run_;
    }
    {
    ...
    sleep(5); // this line just test
    {
        ...
        if (count_writer_wait_ > 0)
            wait_writer_.notify_all();
        else if (count_reader_wait_ > 0)
  ...
  void Reader() {
    {
        std::unique_lock<std::mutex> lock(count_mutex_);
        ++count_reader_wait_;
        wait_reader_.wait(lock, [this]{ return count_writer_run_ + count_writer_wait_ == 0; });
        --count_reader_wait_;
        ++count_reader_run_;
    }
...
template <typename T>
void swap(ReaderWriterAndWriterFirst<T>& lhs, ReaderWriterAndWriterFirst<T>& rhs) {
  lhs.swap(rhs);
}

...
{
	if( bind(connection, servaddr, addrlen) == -1 )
...
{
	if( fgets(st, n, p) == NULL )
		error_exit( -1, "fgets error" );
	if( st[n = ( strlen(st)-1 ) ] == '\n' )
		st[n] = '\0';
	return st;
}

int Connect( int connection, const struct sockaddr* servaddr, socklen_t addrlen )
{

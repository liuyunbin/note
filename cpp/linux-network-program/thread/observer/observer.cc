 ...
 public:
  void add_observer(const std::weak_ptr<Observer>& x) {
    ...
    if (obj)
      ...
      std::cout << "In add_observer, observer had been erase." << std::endl;
  }

  void notifyObservers() {
    std::lock_guard<std::mutex> lock(mutex_);
    for (auto it = std::begin(observer_pool_); it != std::end(observer_pool_); ) {
      std::shared_ptr<Observer> obj(it->lock());
      if (obj) {
        obj->update();
        ++it;
      } else {
        std::cout << "In notifyObservers, observer had been erase." << std::endl;
...
void Observer::add_to_observer_pool(const std::weak_ptr<ObserverPool>& s) {
  ...
  if (obj) {
    ...
    std::cout << "In add_to_observer_pool, ObserverPool had been erase." << std::endl;
  }
}
...
int main() {
  {
    ...
    std::shared_ptr<ObserverPool> observer_pool = std::make_shared<ObserverPool>();

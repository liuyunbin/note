...
void max_heap(void* st, size_t st_len, size_t size, int compare(const void*, const void*), int k) {
	...
	while (1) {
		...
		if (large >= st_len)
			break;
		if (large + 1 < st_len && compare(st + (large + 1) * size, st + large * size) > 0)
			++large;
		if (compare(st + large * size, key) <= 0)
			break;

...
LoggingAsync::LoggingAsync(std::string file_name, std::size_t roll_size,
...
LoggingAsync::~LoggingAsync() { stop(); }
    ...
    while (running_) {
        {
            ...
            if (buffers_.empty()) cond_.wait(lock);
            buffers_.push_back(std::move(current_buffer_));

...
# 总结
## 舍入模式
* fesetround() ----- 存储 => 显示 --- 设置舍入模式
* fegetround() ----- 存储 => 显示 --- 查看舍入模式
* std::floor() ----- 存储 => 显示 --- 向下舍入
* std::ceil() ------ 存储 => 显示 --- 向上舍入
* std::trunc() ----- 存储 => 显示 --- 向零舍入
* std::round() ----- 存储 => 显示 --- 四舍五入 --------- 11.cc
* 最近舍入 --------- 存储 => 显示 --- 四舍六入五取偶 --- 默认
* 浮点数 -> 整数 --- 存储 => 显示 --- 向零舍入
* 编译期 ----------- 显示 => 存储 --- 四舍六入五取偶 --- 04.cc
* 转换后的结果不一定能准确存储

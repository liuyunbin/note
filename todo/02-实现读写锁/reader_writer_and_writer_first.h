...
public:
    ...
    void swap(ReaderWriterAndWriterFirst& rhs) {
        using std::swap;
        {
            ...
            std::lock_guard<std::mutex> rhs_lock(rhs.buf_mutex_,
                                                 std::adopt_lock);
#else
            std::cerr << "ReaderWriterAndWriterFirst need c++11 or later"
                      << std::endl;
    ...
    ReaderWriterAndWriterFirst& operator=(
        const ReaderWriterAndWriterFirst& rhs) {
        auto temp(rhs);
        swap(temp);
        return *this;
    }
    ...
    void Writer(const T& data) {
        {
            ...
            wait_writer_.wait(lock, [this] {
                return count_reader_run_ + count_writer_run_ == 0;
            });
            --count_writer_wait_;
            ++count_writer_run_;
        }
    ...
    void Reader() {
        {
            ...
            wait_reader_.wait(lock, [this] {
                return count_writer_run_ + count_writer_wait_ == 0;
            });
            --count_reader_wait_;
            ++count_reader_run_;
        }
...
template <typename T>
void swap(ReaderWriterAndWriterFirst<T>& lhs,
          ReaderWriterAndWriterFirst<T>& rhs) {
    lhs.swap(rhs);
}

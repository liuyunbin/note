...
public:
    vector<string> letterCombinations(string digits) {
        ...
        if (digits.size() == 0)
            return result;
        const vector<string> number = {"", "", "abc", "def", "ghi", "jkl", "mno", "pqrs", "tuv", "wxyz"};
        string temp;
...
private:
    void backtrack(const string& digits, int k, const vector<string>& number, string& temp, vector<string>& result) {
        if (k == digits.size()) {

...
int find_kth_element(const int* nums1, int nums1_size, const int* nums2, int nums2_size, int k) {
...
double findMedianSortedArrays(int* nums1, int nums1_size, int* nums2, int nums2_size) {
    ...
    double result = find_kth_element(nums1, nums1_size, nums2, nums2_size, nums_size / 2 + 1);
    if (nums_size&1)
        return result;
    result += find_kth_element(nums1, nums1_size, nums2, nums2_size, nums_size / 2);
    return result / 2.0;
}

...
void input() {
    ...
    if (data[0] != '\0') add_history(data);

    // 移除多余的空格
    int k = 0;
    for (size_t i = 0; data[i] != '\0'; ++i)
        if (data[i] != ' ') data[k++] = data[i];
    if (k > 0 && data[k - 1] == '=')
        data[k - 1] = '\0';
    else
        data[k] = '\0';
...
bool count_one_char(char ch) {
    if (stack_long.size() < 2) return false;
...
bool solve() {
    ...
    while (*st != '\0') {
        ...
        if (*st == ')') {
            for (;;) {
                if (stack_char.empty()) return false;
                char ch = stack_char.top();
                stack_char.pop();
                if (ch == '(') break;
                if (count_one_char(ch) == false) return false;
            }
            ++st;
            continue;
        }
        if (*st != '+' && *st != '-' && *st != '*' && *st != '/') return false;
        for (;;) {
    ...
    for (;;) {
        if (stack_char.empty()) break;
        char ch = stack_char.top();
        if (count_one_char(ch) == false) return false;
        stack_char.pop();
    }
    if (stack_long.size() != 1) return false;
...
void output() {
    ...
    if (solve() == false) std::cout << "输入格式错误" << std::endl;
    ...
    while (!stack_long.empty()) stack_long.pop();
}

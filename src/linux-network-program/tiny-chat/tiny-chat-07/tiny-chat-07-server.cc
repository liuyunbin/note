...
std::unordered_map<struct bufferevent *, Node> client;
  ...
  for (;;) {
    ...
    switch (message[0]) {
      case REQUEST_STR: {
        ...
        LOG(INFO) << client[bev].client_message << " " << message;
        ...
        for (auto it = client.begin(); it != client.end(); ++it)
          if (it->first != bev) it->second.message_not_ack.push(message);
        chat::message_unit msg;
        msg.set_user(client[bev].client_message);
        ...
        for (auto it = client.begin(); it != client.end(); ++it)
          if (it->first != bev)
            bufferevent_write(it->first, message.data(), message.size());
        break;
      }
      case HEARTBEAT_STR: {
        chat::heartbeat msg;
        ...
        client[bev].heart_time = msg.time();

        break;
      }
      case RESPONSE_STR: {
        auto &message_not_ack = client[bev].message_not_ack;
        auto &client_message = client[bev].client_message;
        LOG(INFO) << "message: \"" << message_not_ack.front() << "\" to "
                  << client_message << " send success";
        message_not_ack.pop();
        break;
      }
      default: {
...
void event_callback(struct bufferevent *bev, short events, void *arg) {
  if (events & BEV_EVENT_EOF) {
    LOG(INFO) << "client " << client[bev].client_message << " quit";
    client.erase(bev);
    bufferevent_free(bev);
    LOG(INFO) << "current client count: " << client.size();
  } else if (events & BEV_EVENT_ERROR) {
    PLOG(ERROR) << client[bev].client_message;
    client.erase(bev);
    bufferevent_free(bev);
    LOG(INFO) << "current client count: " << client.size();
  }
...
void accept_callback(struct evconnlistener *listener, int sock,
  ...
  struct Node temp;
  temp.client_message = message;
  temp.heart_time = time(NULL);
...
void heartbeat_callback(int fd, short events, void *arg) {
  ...
  while (it != client.end()) {
    int now = time(NULL);
    if (now - HEARTBEAT_SERVER_TIME > it->second.heart_time) {
      LOG(WARNING) << it->second.client_message << " maybe lost, delete";
      auto bev = it->first;
      it = client.erase(it);
      bufferevent_free(bev);
    } else {

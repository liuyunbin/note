...
void do_ttcp(const char *server_ip, uint16_t server_port, int number,
             int length) {
  ...
  if (send_sum < 1024)
    ...
    std::cout << "总共发送：" << send_sum / 1024 << " KiB 数据" << std::endl;
  else if (send_sum < 1024 * 1024 * 1024)
    std::cout << "总共发送：" << send_sum / 1024 / 1024 << " MiB 数据"
              << std::endl;
  else
    std::cout << "总共发送：" << send_sum / 1024 / 1024 / 1024 << " GiB 数据"
              << std::endl;
  ...
  if (speed < 1024)
    ...
    std::cout << "平均速度：" << speed / 1024 / 1024 << " MiB/s " << std::endl;
  else

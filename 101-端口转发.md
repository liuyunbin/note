...
# 2. 使用 rich language 进行端口转发 -- 只允许 非本机访问 (使用 vps 验证)
...
3. sudo firewall-cmd --permanent --policy=portforward --add-ingress-zone=ANY;  # 4. 设置入口网络访问
4. sudo firewall-cmd --permanent --policy=portforward --add-egress-zone=ANY;   # 5. 设置出口
5. sudo firewall-cmd --permanent --policy=portforward --add-rich-rule='rule family="ipv4" forward-port port="4567" protocol="tcp" to-port="1234" to-addr="144.168.57.124"';     # 6. 设置端口转发
6. sudo firewall-cmd --reload;                            # 7. 重新加载防火墙
7. 启动服务: nc -lkv 1234
8. 此时, 4567 不处于监听状态: ss -taln
9.    本机访问: nc 144.168.57.124 4567 -- 失败
10. 非本机访问: nc 144.168.57.124 4567 -- 成功 (实际上是与 1234 相连)

# 3. 使用 rich language 进行端口转发 -- 只允许 本机访问 (使用 vps 验证)
1. sudo firewall-cmd --permanent --delete-policy=portforward; # 1. 删除旧策略 (可选)
2. sudo firewall-cmd --permanent --new-policy=portforward;    # 2. 新增新策略
3. sudo firewall-cmd --permanent --policy=portforward --add-ingress-zone=HOST; # 4. 设置入口本机访问
4. sudo firewall-cmd --permanent --policy=portforward --add-egress-zone=ANY;   # 5. 设置出口
5. sudo firewall-cmd --permanent --policy=portforward --add-rich-rule='rule family="ipv4" forward-port port="4567" protocol="tcp" to-port="1234" to-addr="144.168.57.124"';     # 6. 设置端口转发
6. sudo firewall-cmd --reload;                            # 7. 重新加载防火墙
7. 启动服务: nc -lkv 1234
8. 此时, 4567 不处于监听状态: ss -taln
9. 非本机访问: nc 144.168.57.124 4567 -- 成功 (实际上是与 1234 相连)
10.  本机访问: nc 144.168.57.124 4567 -- 失败
```

...
#include <stdio.h>      // fopen, fclose, fileno, printf, FILE
...
void process_argv_use_cat(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (fp == NULL) {
            error(EXIT_SUCCESS, errno, "cannot open  %s", argv[i]);
            continue;
        }
...
void process_argv_use_more(int argc, char *argv[]) {
    ...
    if (handle_stdin) {
        while (fgets(buf, col_size + 1, stdin) != NULL) {
            ++num_of_lines;
            fputs(buf, stdout);
            if (num_of_lines >= row_size) handle_user_input();
        }
    }
    ...
    for (int i = 1; i < argc; ++i) {
        FILE *fp = fopen(argv[i], "r");
        if (fp == NULL) {
            snprintf(buf, sizeof(buf), "cannot open %s：%s", argv[i],
                     strerror(errno));
            goto failure;
        }

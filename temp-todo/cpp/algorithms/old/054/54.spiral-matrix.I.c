...
int* spiralOrder(int** matrix, int matrixRowSize, int matrixColSize) {
    ...
    if (matrixColSize == 1) {
        for (int i = 0; i < matrixRowSize; ++i) result[i] = matrix[i][0];
        return result;
    }
    int result_index = 0;
    for (int k = 0; k < matrixRowSize / 2 && k < matrixColSize / 2; ++k) {
        ...
        while (i + 1 < matrixRowSize - k)
            result[result_index++] = matrix[i++][j];
        while (j > k) result[result_index++] = matrix[i][j--];
        while (i > k) result[result_index++] = matrix[i--][j];
    }
    if (min(matrixRowSize, matrixColSize) % 2 == 0) return result;

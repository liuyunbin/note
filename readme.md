...
#### 存储布局
```
*         正文段(text) -- 只读, 可共享, 内存中只有一份
*   初始化数据段(data) --   初始化的全局变量或静态变量
* 未初始化数据段(bss)  -- 未初始化的全局变量或静态变量, 将被初始化为 0
* 栈
* 堆
```
...
#### 进程退出
```
* main() 函数返回           -- 和调用 exit() 等价
* 调用 exit()               -- 调用 atexit() 登记的函数, 刷新流, 关闭文件描述符
* 调用 _Exit()  _exit()     -- 调用 atexit() 登记的函数,         关闭文件描述符
* 最后一个线程返回

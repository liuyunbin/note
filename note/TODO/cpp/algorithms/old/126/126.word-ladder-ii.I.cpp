...
class Solution {
   public:
            ...
            for (int i = 0; i < wordList.size(); ++i) {
                ...
                for (int j = 0; j < select.size(); ++j)
                    if (diff(wordList[i], select[j]) == 1) {
                        if (wordList[i] == endWord) if_have_answer = true;
                ...
                if (if_add_i == false) temp_not_select.push_back(wordList[i]);
                if (if_have_answer == true) break;
            }
            if (if_have_answer == true) break;
    ...
    void backtrack(const string& endWord,
                   unordered_map<string, vector<string>>& my_map,
                   list<string>& temp, vector<vector<string>>& result) {

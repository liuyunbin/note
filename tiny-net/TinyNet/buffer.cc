...
std::string Buffer::read() {
    std::string result(
        buffer_.begin() + static_cast<std::ptrdiff_t>(reader_index_),
...
char* Buffer::data() {
    return buffer_.data() + static_cast<std::ptrdiff_t>(reader_index_);
}
std::size_t Buffer::size() {
    return static_cast<std::ptrdiff_t>(writer_index_) -
...
void Buffer::append_head(const char* data, std::size_t len) {
    std::copy(
        data, data + len,
        buffer_.begin() + static_cast<std::ptrdiff_t>(reader_index_ - len));
    reader_index_ -= len;
}

void Buffer::append(const std::string& str) { append(str.data(), str.size()); }

void Buffer::append(const char* data) { append(data, std::strlen(data)); }

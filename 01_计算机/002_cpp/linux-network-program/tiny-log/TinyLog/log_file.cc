...
LogFile::LogFile(std::string file_name, std::size_t roll_size, std::time_t flush_interval) 
...
void LogFile::rollFile(const TimeStamp& val) {
  ...
  char buf[1024];
  std::snprintf(buf, sizeof buf, "%s.%s.%s.%d.log", file_name_.data(), val.data(), buf_hostname, static_cast<int>(::getpid()));

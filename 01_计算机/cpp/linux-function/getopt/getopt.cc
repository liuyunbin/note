...
int   opterr =   1; // 是否输出错误信息，0 表示不输出错误信息，其它值表示要输出错误信息
...
static int process_long_option(int argc, char **argv, const char *optstring,
  ...
  for (const struct option* p = longopts; p->name != NULL; ++p)
    if (strncmp(p->name, d->nextchar, name_len) == 0 && name_len == strlen(p->name))
      found.push_back(p);
...
// 初始化 getopt_data
static const char *getopt_initialize(int argc, char **argv, const char *optstring,
...
#define NONOPTION_P (argv[d->optind][0] != '-' || argv[d->optind][1] == '\0')
  ...
  if (d->nextchar == NULL || *d->nextchar == '\0') {
    ...
    if (longopts) {
      ...
      if (long_only && (argv[d->optind][2] != '\0' || strchr(optstring, argv[d->optind][1]) == NULL)) {
        d->nextchar = argv[d->optind] + 1;
        int code = process_long_option(argc, argv, optstring, longopts, longind,
                                       long_only, d, print_errors, "-");
        if (code != -1) // 解析长选项成功
          return code;
      }
...
static int getopt_internal(int argc, char **argv, const char *optstring,
  ...
  int result = getopt_internal_r(argc, argv, optstring,
                                 longopts, longind,
...
int getopt(int argc, char *const *argv, const char *optstring) {
  return getopt_internal(argc, (char **)argv, optstring,
                         NULL, NULL,
                         0);
}
...
int getopt_long_r(int argc, char **argv, const char *optstring,
                  const struct option *longopts, int *longindex,
                  struct getopt_data *d) {
  return getopt_internal_r(argc, argv, optstring,
                           longopts, longindex,
                           0, d);
}

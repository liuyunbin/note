...
void accept_callback(struct evconnlistener *listener, int sock, struct sockaddr *addr, int len, void *arg) {
  ...
  std::string message = std::string(buf) + ":" + std::to_string(ntohs(addr_in->sin_port));
  printf("new connection from %s\n", message.data());

  auto bev = bufferevent_socket_new(evconnlistener_get_base(listener), sock, BEV_OPT_CLOSE_ON_FREE);

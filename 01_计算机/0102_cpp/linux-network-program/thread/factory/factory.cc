...
class StockFactory : public std::enable_shared_from_this<StockFactory> {
 public:
  std::shared_ptr<Stock> get(const std::string& name) {
    ...
    if (!stock) {
      stock.reset(new Stock(name), std::bind(StockFactory::static_delete_stock,
 ...
 private:
  ...
  static void static_delete_stock(const std::weak_ptr<StockFactory>& weak_factory, Stock* stock) {
    std::shared_ptr<StockFactory> obj(weak_factory.lock());
    if (obj)
      obj->delete_stock(stock);
    delete stock;
  }

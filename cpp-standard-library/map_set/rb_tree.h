#ifndef CPP_STANDARD_LIBRARY_RB_TREE_H_
#define CPP_STANDARD_LIBRARY_RB_TREE_H_
...
struct rb_tree_iterator {
  ...
  rb_tree_iterator(const iterator& other) : node_(other.node_) {}
  ...
  self_iterator& operator++() {
    ...
    if (node_->right_ !=
        parent)  // if node_ == header_ nothing, else node_ = parent
      node_ = parent;
    return *this;
  }
  ...
  self_iterator& operator--() {
    if (node_->color_ == rb_tree_red &&
        node_->parent_->parent_ == node_) {  // node_ == header_
      node_ = node_->right_;
      return *this;
    }
...
rb_tree_node<T>* rb_tree_rebalance_for_erase(rb_tree_node<T>* z,
  ...
  if (y != z) {
    ...
    if (y != z->right_) {
      x_parent = y->parent_;
      if (x != nullptr) x->parent_ = x_parent;
      y->parent_->left_ = x;
      y->right_ = z->right_;
      y->right_->parent_ = y;
    } else {
    ...
    if (left_most == z) {
      if (z->right_ == nullptr) {
        ...
        left_most = z->right_;
        while (left_most->left_ != nullptr) left_most = left_most->left_;
      }
    }
    if (right_most == z) {
      if (z->left_ == nullptr) {
        ...
        right_most = z->left_;
        while (right_most->right_ != nullptr) right_most = right_most->right_;
      }
    }
  }

  if (y->color_ == rb_tree_red) return y;
  ...
  if (x != nullptr) x->color_ = rb_tree_red;
  return y;
}
...
class rb_tree {
 public:
  ...
  iterator erase(const_iterator pos) {
    ...
    auto temp = rb_tree_rebalance_for_erase(pos.node_, header_->parent_,
  ...
  size_type erase(const key_type& key) {
    size_type count = 0;
    auto it = find(key);
    while (it != end() && comp_(key, KeyOfValue()(*it)) == false &&
           comp_(KeyOfValue()(*it), key) == false) {
      it = erase(it);
      ++count;
    }
    return count;
 ...
 private:
  using rb_tree_node_allocator =
      typename Allocator::template rebind<rb_tree_node<value_type>>::other;
  ...
  static const key_type key(rb_tree_node<value_type>* x) {
  ...
  template <typename Y>
  iterator insert_aux(rb_tree_node<value_type>* child,
                      rb_tree_node<value_type>* parent, Y&& value) {
    ...
    if (parent == header_)
      ...
      if (parent == header_->left_) header_->left_ = new_node;
    } else {
      parent->right_ = new_node;
      if (parent == header_->right_) header_->right_ = new_node;
    }
  ...
  iterator insert_equal_aux(Y&& value) {
    ...
    while (child != nullptr) {
      parent = child;
      child = comp_(KeyOfValue()(value), key(parent)) ? parent->left_
                                                      : parent->right_;
    }
  ...
  rb_tree_node<value_type>* copy(const rb_tree_node<value_type>* node) {
    ...
    if (new_node->left_ != nullptr) new_node->left_->parent_ = new_node;
    if (new_node->right_ != nullptr) new_node->right_->parent_ = new_node;
    return new_node;
  }

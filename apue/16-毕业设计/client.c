...
int main(int argc, char **argv) {
    if (argc != 2) error_exit(-1, "argc error");
    ...
    Connect(connection_port, (const struct sockaddr *)(&servaddr_port),
            sizeof(servaddr_port));
    ...
    while (true) {
        ...
        if (strcmp(p1, "exit") == 0 || strcmp(p1, "bye") == 0 ||
            strcmp(p1, "quit") == 0) {
...
static void cmd_ls(void) {
    ...
    Connect(connection_data, (const struct sockaddr *)(&servaddr_data),
            sizeof(servaddr_data));

    while (read(connection_data, output, MAX_SIZE)) printf("%s\t", output);
    printf("\n");

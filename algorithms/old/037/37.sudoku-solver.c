 ...
 *
 */
bool backtrack(char** board, int x, int y, bool row[][9], bool col[][9],
               bool grid[][9]) {
    if (y == 9) {
        y = 0;
        ++x;
    ...
    if (x == 9) return true;
    if (board[x][y] != '.') return backtrack(board, x, y + 1, row, col, grid);
    int k = x / 3 * 3 + y / 3;
    for (int i = 0; i < 9; ++i) {
        ...
        if (backtrack(board, x, y + 1, row, col, grid) == true) return true;
...
void solveSudoku(char** board, int boardRowSize, int boardColSize) {
    ...
    for (int i = 0; i < 9; ++i)
        for (int j = 0; j < 9; ++j) row[i][j] = col[i][j] = grid[i][j] = false;
    for (int i = 0; i < 9; ++i)
        for (int j = 0; j < 9; ++j)
            if (board[i][j] != '.') {
                int num = board[i][j] - '1';
